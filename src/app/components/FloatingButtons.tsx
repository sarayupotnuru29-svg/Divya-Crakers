...
import { useState, useEffect } from 'react';
import { MessageCircle } from 'lucide-react';
// Import your products data (adjust the path if necessary)
...
export function WhatsAppButton() {
  const handleClick = () => {
    ...
    // Generate the product list string without prices
    const productList = products
      .map((item, index) => `${index + 1}. ${item.name}`)
      .join('\n');

    const fullMessage = `Hi, I want to order crackers from Divya Crackers.\n\nOrder List:\n${productList}`;
